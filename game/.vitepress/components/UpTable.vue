        ...
        <tbody class="table-center">
            <tr v-for="(v, i) in rows">
                <td class="td-w-h" v-for="(vv, ii) in v" :style="{ background: getColor(vv.l, vv.m) }">
                    ...
                    <img v-if="vv.char" :src="composeCharSrc(GAME_NAME, vv.char)">
                </td>
...
const wishLength = props.WISH.characters.length;
const characters = props.WISH.characters;
...
const process = () => {
    ...
    versions.value = Object.entries(_versions).map(([ver, len]) => ({
        ver,
        len,
    }));
    ...
    rows.value = [..._rows5, new Array(wishLength).fill({ l: '' }), ..._rows4, new Array(wishLength).fill({ l: '' })];
    names.value = [..._names5, { name: '', length: 0 }, ..._names4, { name: '', length: 0 }];
...
const sortOrder = () => {
    ...
    sort = !sort;
    if (!sort) {
        rows.value = [...__rows5, new Array(wishLength).fill({ l: '' }), ...__rows4];
        names.value = [...__names5, { name: '', length: 0 }, ...__names4];
        return;
    }
...
const filterChar = () => {
    let charMap = new Map();
    for (let wish of props.WISH.characters) {
        //当前时间在祈愿起始时间前
