import { LinkName } from "../../type";

export const dataTravelogue: LinkName[] = [
    { text: '长沙', link: '/trip/travelogue/20220810' },
    { text: '云南', link: '/trip/travelogue/20220812' },
    { text: '苏州', link: '/trip/travelogue/20220906' },
    { text: '南京', link: '/trip/travelogue/20230225' },
    { text: '上海', link: '/trip/travelogue/20230311' },
    { text: '舟山', link: '/trip/travelogue/20230415' },
    { text: '绍兴', link: '/trip/travelogue/20230422' },
    { text: '港珠澳广深', link: '/trip/travelogue/20230429' },
    { text: '长春', link: '/trip/travelogue/20230527' },
    { text: '南宁', link: '/trip/travelogue/20230610' },
    { text: '深港珠+柳州', link: '/trip/travelogue/20230628' },
    { text: '日本', link: '/trip/travelogue/20230720' },
    { text: '青岛淄博济宁', link: '/trip/travelogue/20230803' },
    { text: '南京', link: '/trip/travelogue/20230809' },
    { text: '桂林', link: '/trip/travelogue/20230902' },
    { text: '潮汕地区', link: '/trip/travelogue/20240404' },
    { text: '海口', link: '/trip/travelogue/20240415' },
    { text: '贵阳', link: '/trip/travelogue/20240426' },
];
